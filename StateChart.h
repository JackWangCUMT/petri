...
public:
	...
	std::shared_ptr<Condition> compareResult(ResultatAction const &r) const {
...
public:
	StateChart() : _actionsPool(InitialThreadsActions) {
	...
	void run() {
		...
		_running = true;
		_statesManager = std::thread(&StateChart::manageStates, this);
	}
		...
		while(_running) {
			...
			for(auto it = _toBeActivated.begin(); it != _toBeActivated.end(); ) {
				...
				if(a.currentTokens() >= a.requiredTokens()) {
					...
					_actionsPool.addTask(make_callable_ptr(&StateChart::executeState, *this, std::ref(a)));

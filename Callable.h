...
#ifndef IA_Pe_tri_Callable_h
#define IA_Pe_tri_Callable_h
...
public:
	Callable(CallableType const &c, Args &&...args) : _c(c), _args(std::forward<Args>(args)...) {}
...
auto make_callable(CallableType &&c, Args &&...args) {
	...
	return Callable<ComputedReturnType, CallableType, Args...>(c, std::forward<Args>(args)...);
}
...
auto make_callable_ptr(CallableType &&c, Args &&...args) {
	...
	return static_cast<std::shared_ptr<CallableBase<ComputedReturnType>>>(std::make_shared<Callable<ComputedReturnType, CallableType, Args...>>(c, std::forward<Args>(args)...));
}

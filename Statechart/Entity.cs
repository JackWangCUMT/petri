...
namespace Statechart
{
	...
	{
		public Entity(Document doc, StateChart parent) {
			this.parent = parent;
			this.Document = doc;
			this.ID = Document.LastEntityID++;
		}

		public static Entity EntityFromXml(Document document, XElement descriptor, StateChart parent, IDictionary<UInt64, State> statesTable) {
			...
			case "StateChart":
				if(parent == null)
					return new RootStateChart(document, descriptor);
				else
					return new InnerStateChart(document, parent, descriptor);
			case "Transition":
		...
		public Entity(Document doc, StateChart parent, XElement descriptor) {
			this.parent = parent;
			this.Document = doc;

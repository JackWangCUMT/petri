		...
		public abstract class Expression
		{
			...
			private static Expression CreateFromPreprocessedString(string s, Entity entity, IEnumerable<Function> funcList, Dictionary<int, Tuple<ExprType, string>> subexprs) {
				for(int i = 17; i >= 0; --i) {
					int bound;
					int direction;
					if(Cpp.Operator.Properties[Cpp.Operator.ByPrecedence[i][0]].associativity == Statechart.Cpp.Operator.Associativity.LeftToRight) {
						bound = s.Length;
						direction = 1;
					}
					else {
					...
					int index = bound;
					var foundOperator = Statechart.Cpp.Operator.Name.None;
					foreach(var op in Cpp.Operator.ByPrecedence[i]) {
						...
						if(prop.implemented) {
							...
							if(opIndex.CompareTo(index) == -direction) {
								...
								if(prop.ambiguous) {
									if(prop.type == Statechart.Cpp.Operator.Type.PrefixUnary) {
										...
										foreach(var op2 in Enum.GetValues(typeof(Cpp.Operator.Name)).Cast<Cpp.Operator.Name>()) {
											if(Cpp.Operator.Properties[op2].type == Statechart.Cpp.Operator.Type.Binary) {
					...
					if(index != bound) {
						...
						if(prop.type == Statechart.Cpp.Operator.Type.Binary) {
							string e1 = s.Substring(0, index);
							string e2 = s.Substring(index + prop.cpp.Length);

							if(foundOperator == Statechart.Cpp.Operator.Name.SelectionRef || foundOperator == Statechart.Cpp.Operator.Name.SelectionPtr) {
						...
						else if(prop.type == Statechart.Cpp.Operator.Type.PrefixUnary) {
						...
						else if(prop.type == Statechart.Cpp.Operator.Type.SuffixUnary) {
							if(foundOperator == Statechart.Cpp.Operator.Name.FunCall) {
			...
			protected static string Parenthesize(Expression parent, Expression child, string representation) {
				...
				if(child.Operator != Cpp.Operator.Name.None) {
					...
					if(parentProperties.precedence < childProperties.precedence) {
						...
						if(parentProperties.type == Statechart.Cpp.Operator.Type.Binary) {
							...
							if(parentProperties.associativity == Statechart.Cpp.Operator.Associativity.LeftToRight && castedParent.Expression2 == child) {
								parenthesize = true;
							}
							// If the operator is right-associative, but the expression was parenthesize from left to right
							else if(parentProperties.associativity == Statechart.Cpp.Operator.Associativity.RightToLeft && castedParent.Expression1 == child) {
								parenthesize = true;
							}
						}
					}

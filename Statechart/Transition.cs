...
namespace Statechart
{
	...
	{
		public Transition(Document doc, StateChart s, State before, State after) : base(doc, s)
		{
			this.before = before;
			this.after = after;
			...
			this.Condition = new CheckResultCondition(this, Action.ResultatAction.ActionReussie);

			this.UpdatePosition();
		}

		public Transition(Document doc, StateChart parent, XElement descriptor, IDictionary<UInt64, State> statesTable, List<Cpp.Function> conditions) : base(doc, parent, descriptor) {
		...
		public override void GenerateCpp(Cpp.Generator source, IDManager lastID) {
			...
			var b = this.Before as InnerStateChart;
			if(b != null) {
				bName = b.ExitPoint.CppName;
			}

			var a = this.After as InnerStateChart;
			if(a != null) {
